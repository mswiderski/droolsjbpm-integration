...
public class RuleCapabilitiesServiceImplTest extends AbstractServiceImplTest {
    ...
    public void testScanNow() {
        ...
        when(kieServerInstanceManager.scanNow(any(), any())).thenReturn(fakeResult);

        ruleCapabilitiesService.scanNow(containerSpec);

        verify(kieServerInstanceManager, times(1)).scanNow(any(), any());
    }
    ...
    public void testStartScanner() {
        ...
        List<Container> fakeResult = new ArrayList<Container>();
        fakeResult.add(container);
        when(kieServerInstanceManager.startScanner(any(), any(), anyLong())).thenReturn(fakeResult);

        ruleCapabilitiesService.startScanner(containerSpec, 100l);

        verify(kieServerInstanceManager, times(1)).startScanner(any(), any(), anyLong());
    ...
    public void testStopScanner() {
        List<Container> fakeResult = new ArrayList<Container>();
        fakeResult.add(container);
        when(kieServerInstanceManager.stopScanner(any(), any())).thenReturn(fakeResult);

        ruleCapabilitiesService.stopScanner(containerSpec);

        verify(kieServerInstanceManager, times(1)).stopScanner(any(), any());
    ...
    public void testUpgradeContainer() {
        ...
        List<Container> fakeResult = new ArrayList<Container>();
        fakeResult.add(container);
        when(kieServerInstanceManager.upgradeContainer(any(), any())).thenReturn(fakeResult);
        ...
        verify(kieServerInstanceManager, times(1)).upgradeContainer(any(), any());
