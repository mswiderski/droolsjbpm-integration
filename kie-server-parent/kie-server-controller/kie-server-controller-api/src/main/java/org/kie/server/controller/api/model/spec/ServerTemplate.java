...
public class ServerTemplate extends ServerTemplateKey {
    ...
    public boolean hasServerInstance(String serverInstanceId) {
        for (ServerInstanceKey spec : getServerInstanceKeys()) {
            if (serverInstanceId.equals(spec.getServerInstanceId())) {
                return true;
            }
        }
        return false;
    ...
    public void addServerInstance(String url) {
        if (serverInstances == null) {
            serverInstances = new ArrayList<ServerInstanceKey>();
        }
        ServerInstanceKey serverInstance = new ServerInstanceKey(this.getName()+"@"+url, this.getName(), this.getId(), url);
